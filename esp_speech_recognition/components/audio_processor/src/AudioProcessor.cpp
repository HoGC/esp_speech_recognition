...
void AudioProcessor::get_spectrogram(int16_t *buffer, float *output_spectrogram)
{
    ...
    for (int i = 0; i < m_audio_length; i++)
    {
        max = std::max(max, fabsf((float)(buffer[i]) - mean));
    }
    ...
    for (int window_start = 0; window_start < 16000 - m_window_size; window_start += m_step_size)
    {
        ...
        {
            m_fft_input[i] = ((float)(buffer[window_start + i]) - mean) / max;
        }
